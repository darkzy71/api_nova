...
class YuriCanvas {
    ...
    static async rank(
        ...
        if (!options.color || typeof options.color !== "string") options.color = "#ff0000";
        ...
        ctx.fillStyle = color || "#ff0000";
        ctx.fillText("NIVEL", 934 - 64 - ctx.measureText(level).width - 16, 82);
     ...
     * canva.write(img, "img.png");
     */
    static async welcome({ username, discrim, avatarURL, fundo, titulo, cor1, cor2 }) {
        ...
        if (!titulo) throw new Error("No titulo was provided!");                
     ...
     * canva.write(img, "img.png");
     */
    static async leaver({ username, discrim, avatarURL, fundo, color, titulo, cor1, cor2 }) {
        if (!username) throw new Error("No username was provided!");
        if (!discrim) throw new Error("No discrim was provided!");
        if (!avatarURL) throw new Error("No avatarURL was provided!");
        if (!titulo) throw new Error("No titulo was provided!");        
